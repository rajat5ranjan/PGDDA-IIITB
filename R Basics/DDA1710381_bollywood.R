...
#	Import the Bollywood data set in Rstudio in a variable named bollywood
  ## Comment from Anugraha : Using read.csv function, with stringAsFactor = False ##
...
# Store the movie name in variable named top_movie
  ...
  top_movie <- bollywood[order(bollywood$Tcollection,decreasing = TRUE)[1],'Movie']
...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <- bollywood[order(bollywood$Tcollection,decreasing = TRUE)[2],'Movie']
...
# Store the names of 4 movies in same sequence in movie_result vector
	...
	movie_result <- sapply(c("Ocollection","Wcollection","Fwcollection","Tcollection"),function(x) bollywood[order(bollywood[,eval(x)],decreasing = TRUE)[1],'Movie'])
