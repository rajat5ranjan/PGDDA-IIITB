...
# 2) Lets check whether all column names match in both in_time and out_time are same
...
# them for now, we will come back to them later
...
# For other columns where NAs are present, there is no obvious relationship that can be drawn.
# Also data does not show any specific relationship/pattern with other columns
...
# Based on above analysis, if we remove the rows from this data frame, then
...
# Final Cleaned up data saved as emp_data        
...
G19 <- ggplot() + geom_line(aes(x=c(0:100),y=quantile(emp_data$MonthlyIncome,seq(0,1,0.01)))) + 
  scale_y_continuous(breaks=seq(0,200000,10000)) +
  labs(title="G19 - MonthlyIncome Quantile Rise",x="Quantile",y="MonthlyIncome")
grid.arrange(G19)
...
G31 <- ggplot() + 
  ...
  labs(title="G31 - TotalWorkingYears quantile wise increase",x="quantile",y="TotalWorkingYears")
grid.arrange(G31)
...
G34 <- ggplot() + 
  ...
  scale_x_continuous(breaks=seq(0,100,7)) +
  scale_y_continuous(breaks=seq(0,40,2)) +
  labs(title="G34 - YearsAtCompany quantile wise increase",x="quantile",y="YearsAtCompany")
grid.arrange(G34)
...
G37 <- ggplot() + 
  ...
  scale_x_continuous(breaks=seq(0,100,7)) +
  scale_y_continuous(breaks=seq(0,40,2)) +
  labs(title="G37 - YearsSinceLastPromotion quantile wise increase",x="quantile",y="YearsSinceLastPromotion")
grid.arrange(G37)
...
G42 <- ggplot() + 
  ...
  scale_x_continuous(breaks=seq(0,100,7)) +
  scale_y_continuous(breaks=seq(0,40,2)) +
  labs(title="G42 YearsWithCurrentManager quantile wise increase",x="quantile",y="YearsWithCurrManager")
grid.arrange(G42)
...
# Based on the above model, following significant variables have been seen and inference from
# their co-efficients can be drawn as follows
...
# percYearlyWorking                  0.58747    0.05420  10.839  < 2e-16 *** 
# Positive coefficient:More working hours = higher attrition probability

# Age                               -0.26054    0.07865  -3.313 0.000924 ***
# Negative coefficient  : Higher the age = lesser attrition probability

# NumCompaniesWorked                 0.36017    0.05742   6.272 3.56e-10 ***
# Positive coefficient    : Employee who switches job frequently = higher attrition probability

# TotalWorkingYears                 -0.64254    0.10449  -6.149 7.79e-10 ***
# Negative coefficient  : Higher the total experience of employee = lesser the attrition probability

# TrainingTimesLastYear             -0.20913    0.05761  -3.630 0.000283 ***
# Negative coefficient  :   More the trainings = lesser is the attrition probability

# YearsSinceLastPromotion            0.62684    0.08711   7.196 6.19e-13 ***
# Positive coefficient    :   Later the promotion = higher attrition probability

# YearsWithCurrManager              -0.58090    0.09462  -6.139 8.29e-10 ***
# Negative coefficient   : Changing the manager frequently = higher attrition probability

# EnvironmentSatisfaction           -0.38163    0.05548  -6.879 6.03e-12 ***
# Negative coefficient   : Higher the env satisfaction = lesser is attrition probability

# JobSatisfaction                   -0.39377    0.05649  -6.971 3.16e-12 ***
# Negative coefficient   : Higher is the job satisfaction = lesser is the attrition probability

# WorkLifeBalance                   -0.24788    0.05461  -4.539 5.64e-06 ***
# Negative coefficient   : Higher is work life balance = lesser is the attrition probability

# BusinessTravel.xTravel_Frequently  0.78395    0.13005   6.028 1.66e-09 ***
# Positive coefficient     : More business trips = higher attrition probability

# JobRole.xManufacturing.Director   -0.92506    0.22247  -4.158 3.21e-05 ***
# Negative coefficient   : Higher job role = lesser attrition probability

# MaritalStatus.xSingle              1.03141    0.11525   8.949  < 2e-16 ***
# Positive Coefficient    : Single employees = higher attrition probability
#------------------------------------------------------
#
# Model Evaluation Conclusions :
# Based on the comparisons done above, we have concluded that KS Statistics is a better
# metric for evaluating the model. The confusionMatrix for KS Statistics states the following
#             Reference
# Prediction    0    1
#          0   737  48
#          1   346 161
#
# One of the inferred meaning from high AUC is that we are able to tap high number of 
# actual attrition employees. However, in doing so, we are also targeting some employees
# which are less likely to resign. However, as per the business problem, it is more important
# to tap those employees which are most likely to resign.
# Hence on those lines, KS statistics shows a better approach for achieving this business objective.
